...
def vocab_config(vocab_config, mode, Glove_path, results_path):
  config = _HParams()
  ...
  if config.mode == 'fixed' and mode != "eval":
    config.vocab_file = os.path.join(Glove_path, 'glove.840B.300d.txt')
    config.embs_file = os.path.join(Glove_path, 'glove.840B.300d.npy')
  elif mode == "encode" and config.mode == "trained":
    config.vocab_file = vocab_config['vocab_file']
  elif mode == "encode" and config.mode == "expand":
    config.vocab_file = os.path.join(results_path, vocab_config['vocab_file'])
    config.embs_file = os.path.join(results_path, vocab_config['embs_file'])
  return config

def model_config(mdl_config, mode, Glove_path, results_path):
  ...
  if mdl_config['checkpoint_path']:
    config.checkpoint_path = os.path.join(results_path, mdl_config['checkpoint_path'])
  config.vocab_configs = []
  for vocab_configs in mdl_config['vocab_configs']:
    config.vocab_configs.append(vocab_config(vocab_configs, mode, Glove_path, results_path))
