...
class SkipThoughtsModel(object):
  ...
  def build_word_embeddings(self):
    ...
    if self.mode == "encode":
      ...
      if self.config.word_embedding_file:
        # transfer learning: load pre-trained word vectors
        word_vecs = np.load(self.config.word_embedding_file)
        init = tf.constant(word_vecs)
        print('Use pre-trained word vectors %d', word_vecs.shape)
