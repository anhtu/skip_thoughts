...
from .ops import s2v_input_ops as input_ops
...
class s2v(object):
    ...
    def __init__(self, config, mode="train", input_reader=None, input_queue=None, model_params=None):
    ...
    def build_word_embeddings(self):
        tf.logging.info('=== build_word_embeddings ===')
        rand_init = self.uniform_initializer
        self.word_embeddings = []
        self.encode_emb = []
        self.init = None

        # there might be more than one vocab configs in json file
        for v in self.config.vocab_configs:

            # Pre-trained word vectos 
            if v.mode == 'fixed':
                if self.mode == "train":
                    ...
                    # adding a random vector to the first dim
            ...
            # Training word vectors from scratch
            if v.mode == 'trained':
                # create two embedding variables `word_embedding` and `word_embedding_out`
                # add them to `word_embeddings` attr
                # also make two lookup for `encode_ids`
                for inout in ["", "_out"]:
                    word_emb = tf.get_variable(
                        name=v.name + inout,     
                        shape=[v.size, v.dim],
    ...
    def _initialize_cell(self, num_units, cell_type="GRU"):
        tf.logging.info('=== _initialize_cell ===')
        if cell_type == "GRU":
    ...
    def build_encoder(self):
        ...
        Inputs:
            self.encode_emb - word vectors looked up
            self.encode_mask
    ...
    def build_loss(self):
        ...
        ctxt_sent_pos = [i for i in range(-self.context_size, self.context_size + 1)]
